class Application {
    ...
    setSharingOptions(){
        var form_url = window.location.hostname + php_self + "forms/" + this.current_form_path.split("/")[this.current_form_path.split("/").length - 1];
    ...
    formBuilderSave(form_txt){
        ...
        $.ajax({
            url: "../contact_form/save_form.php",
            cache: false,
            context: this,
            method: "POST",
            data: {
    ...
    bindAppendButtons(){
        $(".editor-btn").click(function(){
            ...
            var url = "../contact_form/templates/" + $(this).attr("data-type") + ".txt";
            $.get({
                url: url,
                context:this,
                cache: false
